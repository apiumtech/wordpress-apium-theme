    <?php
/**
 * Created by JetBrains PhpStorm.
 * User: xavi
 * Date: 6/2/13
 * Time: 9:38 PM
 * To change this template use File | Settings | File Templates.
 */



?>
<div id="weke">


    <ul id="dynamicFrontEnd" style="top: -0" ng-cloak>

        <li>
weke
        </li>

    </ul>

</div>

<!-- CONTENT WHAT APIUM-->
<div class="container-fluid apiumwhatfluid">

    <div class="container whatapium hero-unit">
        <h1 class="text-center">Hi! We are <b>Apium Tech</b> team
            <span class="tagline text-center">Fresh, innovative and worthy IT Consulting</span></h1>
        <hr />
        <h2 class="text-center">What we can do for you and your business</h2>

        <!-- carrousel only mobile -->
        <div id="myCarousel" class="visible-phone carousel slide">

            <!-- Carousel items -->
            <div class="carousel-inner">
                <div class="active item">

                    <h3><i class="mvp"> </i>Get your idea done in record time</h3>
                    <p>We planify, priorize and develop to assure you a Minimum Viable Product launched in 30-90 days.</p>
                    <p><a href="#contactus" class="btn btnapium btn-block btn-primary" type="button">Contact us to know more</a></p>
                </div>
                <div class="item">

                    <h3><i class="efficiency"> </i>Get the most of your product</h3>
                    <p>When you grow, scalability and architecture become essential and we can help you to build your success.</p>
                    <p><a href="#contactus" class="btn btnapium btn-block btn-primary" type="button">Contact us to know more</a></p>
                </div>
                <div class="item">

                    <h3><i class="multiplatform"> </i>Go faster, reach everyone</h3>
                    <p>At these days reaching customers at any time and at any device is a must. Be  multiplatform from the beginning.</p>
                    <p><a href="#contactus" class="btn btnapium btn-block btn-primary" type="button">Contact us to know more</a></p>
                </div>
                <div class="item">

                    <h3><i class="usertest"> </i>Get customer feedback right away</h3>
                    <p>Customers are the right voice to listen to. We provide feedback and research even before development starts.</p>
                    <p><a href="#contactus" class="btn btnapium btn-block btn-primary" type="button">Contact us to know more</a></p>
                </div>
            </div>

            <ol class="carousel-indicators">
                <li data-target="#myCarousel" data-slide-to="0" class="active"></li>
                <li data-target="#myCarousel" data-slide-to="1"></li>
                <li data-target="#myCarousel" data-slide-to="2"></li>
                <li data-target="#myCarousel" data-slide-to="3"></li>
            </ol>

        </div>



        <div class="hidden-phone row-fluid">
            <div class="span3">
                <i class="mvp"></i>
                <h3>Get your idea done in record time</h3>
                <p>We planify, priorize and develop to assure you a Minimum Viable Product launched in 30-90 days.</p>
                <p><a href="#contactus" class="btn btnapium btn-block btn-primary" type="button">Contact us to know more</a></p>
            </div>
            <div class="span3">
                <i class="efficiency"></i>
                <h3>Get the most of your product</h3>
                <p>When you grow, scalability and architecture become essential and we can help you to build your success.</p>
                <p><a href="#contactus" class="btn btnapium btn-block btn-primary" type="button">Contact us to know more</a></p>
            </div>
            <div class="span3">
                <i class="multiplatform"></i>
                <h3>Go faster, reach everyone</h3>
                <p>At these days reaching customers at any time and at any device is a must. Be  multiplatform from the beginning.</p>
                <p><a href="#contactus" class="btn btnapium btn-block btn-primary" type="button">Contact us to know more</a></p>
            </div>
            <div class="span3">
                <i class="usertest"></i>
                <h3>Get customer feedback right away</h3>
                <p>Customers are the right voice to listen to. We provide feedback and research even before development starts.</p>
                <p><a href="#contactus" class="btn btnapium btn-block btn-primary" type="button">Contact us to know more</a></p>
            </div>
        </div><!-- end row fluid -->
        <hr />
        <h2 class="text-center">Why choosing us? Let us introduce to us</h2>
        <p class="lead text-justify">We are an involved, young and passionate team with more than 10 years experience in new technologies and methodogies. "Updated, innovative and committed" are words that define us properly. We don't inflate your budget and the most important thing we give you technologic solutions and real customers feedback for your present thinking also in your future.</p>
        <!-- testimonial only mobile -->
        <!-- carrousel only mobile -->
        <div id="myCarousel" class="visible-phone carousel slide">

            <!-- Carousel items -->
            <div class="carousel-inner">
                <div class="active item">
                    <blockquote>
                        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer posuere erat a ante.</p>
                        <small>Someone famous <cite title="Source Title">Source Title</cite></small>
                    </blockquote>
                </div>
                <div class="item">
                    <blockquote>
                        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer posuere erat a ante.</p>
                        <small>Someone famous <cite title="Source Title">Source Title</cite></small>
                    </blockquote>
                </div>
                <div class="item">
                    <blockquote>
                        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer posuere erat a ante.</p>
                        <small>Someone famous <cite title="Source Title">Source Title</cite></small>
                    </blockquote>
                </div>
            </div>

            <ol class="carousel-indicators">
                <li data-target="#myCarousel" data-slide-to="0" class="active"></li>
                <li data-target="#myCarousel" data-slide-to="1"></li>
                <li data-target="#myCarousel" data-slide-to="2"></li>
            </ol>

        </div><!-- end carrousel -->


        <div class="row-fluid hidden-phone">
            <div class="span4">
                <blockquote>
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer posuere erat a ante.</p>
                    <small>Someone famous <cite title="Source Title">Source Title</cite></small>
                </blockquote>
            </div>
            <div class="span4">
                <blockquote>
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer posuere erat a ante.</p>
                    <small>Someone famous <cite title="Source Title">Source Title</cite></small>
                </blockquote>
            </div>
            <div class="span4">
                <blockquote>
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer posuere erat a ante.</p>
                    <small>Someone famous <cite title="Source Title">Source Title</cite></small>
                </blockquote>
            </div>
        </div><!-- end row fluid -->
    </div><!-- end whatapium -->
    <p class="scroll text-center">
        <a class="hidden-phone" href="#whoarewe"><i class="next">&raquo;</i> Who are we?</a>
        <a href="#"><i class="top">&raquo;</i> Go to the top</a></p>
</div><!-- end apiumwhatfluid -->

<!-- WHO APIUM -->
<div class="container-fluid apiumwhofluid" id="whoarewe">
    <div class="container whoapium hero-unit">
        <h2 class="text-center">Who are we? First of all, "Nice to meet you!" :-)</h2>

        <!-- testimonial only mobile -->
        <!-- carrousel only mobile -->
        <div id="myCarousel" class="visible-phone carousel slide">

            <!-- Carousel items -->
            <div class="carousel-inner">
                <div class="active item">
                    <blockquote class="whoblockquote">

                        <small>
                            <i class="xavi"></i>
                            <strong>Xavi Hidalgo</strong><br>
                            CEO<br>Chief Technology Officer<br>
                        </small>

                    </blockquote>
                    <p class="clearfix phrase">"It's amazing how different disciplines can come together and help us find unexpected solutions to common problems"</p>

                    <ul class="unstyled contact">
                        <li><i class="twitter"></i><a href="https://twitter.com/xavi_reloaded">@xavi_reloaded</a></li>
                        <li><i class="linkedin"></i><a href="http://es.linkedin.com/in/xavireloaded">es.linkedin.com/in/xavireloaded</a></li>
                        <li><i class="phone"></i>+34 633 029 763</li>
                        <li><i class="email"></i><a href="mailto:xavi.hidalgo@apiumtech.com">xavi.hidalgo@apiumtech.com</a></li>
                    </ul>
                    <div class="tags">
                        <span class="label">TDD</span> <span class="label">AGILE</span> <span class="label">TESTACULAR</span> <span class="label">ANGULAR</span> <span class="label">SCRUM</span> <span class="label">TEAMLEAD</span> <span class="label">COACHING</span> <span class="label">NOSQL</span> <span class="label">MUMPS</span> <span class="label">CODING DOJO</span> <span class="label">JAVASCRIPT</span><span class="label">MOBILE APPLICATIONS</span> <span class="label">ANDROID</span> <span class="label">IOS</span> <span class="label">WEB APPLICATIONS</span> <span class="label">ENTERPRISE PATTERNS</span> <span class="label">SPRING</span>
                    </div>
                </div>
                <div class="item">
                    <blockquote class="whoblockquote">
                        <small>
                            <i class="christian"></i>
                            <strong>Christian Ciceri</strong><br>
                            Co-Founder<br>Software Architect Manager<br>
                        </small>
                    </blockquote>

                    <p class="clearfix phrase">"An architect should be capable of switching between high-level and low-level aspects in a natural, quick and continuous way"</p>


                    <ul class="unstyled contact">
                        <li><i class="twitter"></i><a href="#">This guy is shy! Too bad, he knows a lot!!</a></li>
                        <li><i class="linkedin"></i><a href="http://es.linkedin.com/in/cciceri">es.linkedin.com/in/cciceri</a></li>
                        <li><i class="phone"></i>+34 697 864 292</li>
                        <li><i class="email"></i><a href="mailto:christian.ciceri@apiumtech.com">christian.ciceri@apiumtech.com</a></li>
                    </ul>

                    <div class="tags">
                        <span class="label">TDD</span> <span class="label">AGILE</span> <span class="label">TESTACULAR</span> <span class="label">ANGULAR</span> <span class="label">SCRUM</span> <span class="label">TEAMLEAD</span> <span class="label">COACHING</span> <span class="label">NOSQL</span> <span class="label">MUMPS</span> <span class="label">CODING DOJO</span> <span class="label">JAVASCRIPT</span><span class="label">MOBILE APPLICATIONS</span> <span class="label">ANDROID</span> <span class="label">IOS</span> <span class="label">WEB APPLICATIONS</span> <span class="label">ENTERPRISE PATTERNS</span> <span class="label">SPRING</span>
                    </div>
                </div>
                <div class="item">
                    <blockquote class="whoblockquote">
                        <small>
                            <i class="laura"></i>
                            <strong>Laura Vilar</strong><br>
                            Co-Founder<br>Visual and UX Designer<br>
                        </small>
                    </blockquote>

                    <p class="clearfix phrase">"I love the way simple design, user thinking knowledge and design process optimization combines perfectly to assure good results"</p>

                    <ul class="unstyled contact">
                        <li><i class="twitter"></i><a href="https://twitter.com/lauravilarruiz">@lauravilarruiz</a></li>
                        <li><i class="linkedin"></i><a href="http://es.linkedin.com/in/lauravilarruiz">es.linkedin.com/in/lauravilarruiz</a></li>
                        <li><i class="phone"></i>+34 645 765 199</li>
                        <li><i class="email"></i><a href="mailto:laura.vilar@apiumtech.com">laura.vilar@apiumtech.com</a></li>
                    </ul>

                    <div class="tags">
                        <span class="label">DESIGN</span> <span class="label">INTERACTION</span> <span class="label">USABILITY</span> <span class="label">WEB</span> <span class="label">LEAN UX</span> <span class="label">USER EXPERIENCE</span> <span class="label">APPLICATION DESIGN</span> <span class="label">CSS</span> <span class="label">HTML</span> <span class="label">LESS</span> <span class="label">BOOTSTRAP</span><span class="label">USER TEST</span> <span class="label">USER RESEARCH</span> <span class="label">WIREFRAMING</span> <span class="label">PROTOTYPING</span>  <span class="label">PRODUCT MANAGER</span> <span class="label">AGILE</span>
                    </div>
                </div>
            </div>

            <ol class="carousel-indicators">
                <li data-target="#myCarousel" data-slide-to="0" class="active"></li>
                <li data-target="#myCarousel" data-slide-to="1"></li>
                <li data-target="#myCarousel" data-slide-to="2"></li>
            </ol>

        </div><!-- end carrousel -->


        <div class="row-fluid hidden-phone">
            <!-- XAVI -->
            <div class="span4">
                <blockquote class="whoblockquote">
                    <small>
                        <i class="xavi"></i>
                        <strong>Xavi Hidalgo</strong><br>
                        CEO<br>Chief Technology Officer<br>
                    </small>

                </blockquote>
                <p class="clearfix phrase">"It's amazing how different disciplines can come together and help us find unexpected solutions to common problems"</p>

                <ul class="unstyled contact">
                    <li><i class="twitter"></i><a href="https://twitter.com/xavi_reloaded">@xavi_reloaded</a></li>
                    <li><i class="linkedin"></i><a href="http://es.linkedin.com/in/xavireloaded">es.linkedin.com/in/xavireloaded</a></li>
                    <li><i class="phone"></i>+34 633 029 763</li>
                    <li><i class="email"></i><a href="mailto:xavi.hidalgo@apiumtech.com">xavi.hidalgo@apiumtech.com</a></li>
                </ul>
                <div class="tags">
                    <span class="label">TDD</span> <span class="label">AGILE</span> <span class="label">TESTACULAR</span> <span class="label">ANGULAR</span> <span class="label">SCRUM</span> <span class="label">TEAMLEAD</span> <span class="label">COACHING</span> <span class="label">NOSQL</span> <span class="label">MUMPS</span> <span class="label">CODING DOJO</span> <span class="label">JAVASCRIPT</span><span class="label">MOBILE APPLICATIONS</span> <span class="label">ANDROID</span> <span class="label">IOS</span> <span class="label">WEB APPLICATIONS</span> <span class="label">ENTERPRISE PATTERNS</span> <span class="label">SPRING</span>
                </div>
            </div>

            <!-- CHRISTIAN -->
            <div class="span4">
                <blockquote class="whoblockquote">
                    <small>
                        <i class="christian"></i>
                        <strong>Christian Ciceri</strong><br>
                        Co-Founder<br>Software Architect Manager<br>
                    </small>
                </blockquote>

                <p class="clearfix phrase">"An architect should be capable of switching between high-level and low-level aspects in a natural, quick and continuous way"</p>


                <ul class="unstyled contact">
                    <li><i class="twitter"></i><a href="#">This guy is shy! Too bad, he knows a lot!!</a></li>
                    <li><i class="linkedin"></i><a href="http://es.linkedin.com/in/cciceri">es.linkedin.com/in/cciceri</a></li>
                    <li><i class="phone"></i>+34 697 864 292</li>
                    <li><i class="email"></i><a href="mailto:christian.ciceri@apiumtech.com">christian.ciceri@apiumtech.com</a></li>
                </ul>

                <div class="tags">
                    <span class="label">TDD</span> <span class="label">AGILE</span> <span class="label">TESTACULAR</span> <span class="label">ANGULAR</span> <span class="label">SCRUM</span> <span class="label">TEAMLEAD</span> <span class="label">COACHING</span> <span class="label">NOSQL</span> <span class="label">MUMPS</span> <span class="label">CODING DOJO</span> <span class="label">JAVASCRIPT</span><span class="label">MOBILE APPLICATIONS</span> <span class="label">ANDROID</span> <span class="label">IOS</span> <span class="label">WEB APPLICATIONS</span> <span class="label">ENTERPRISE PATTERNS</span> <span class="label">SPRING</span>
                </div>

            </div>

            <!-- LAURA -->
            <div class="span4">
                <blockquote class="whoblockquote">
                    <small>
                        <i class="laura"></i>
                        <strong>Laura Vilar</strong><br>
                        Co-Founder<br>Visual and UX Designer<br>
                    </small>
                </blockquote>

                <p class="clearfix phrase">"I love the way simple design, user thinking knowledge and design process optimization combines perfectly to assure good results"</p>

                <ul class="unstyled contact">
                    <li><i class="twitter"></i><a href="https://twitter.com/lauravilarruiz">@lauravilarruiz</a></li>
                    <li><i class="linkedin"></i><a href="http://es.linkedin.com/in/lauravilarruiz">es.linkedin.com/in/lauravilarruiz</a></li>
                    <li><i class="phone"></i>+34 645 765 199</li>
                    <li><i class="email"></i><a href="mailto:laura.vilar@apiumtech.com">laura.vilar@apiumtech.com</a></li>
                </ul>

                <div class="tags">
                    <span class="label">DESIGN</span> <span class="label">INTERACTION</span> <span class="label">USABILITY</span> <span class="label">WEB</span> <span class="label">LEAN UX</span> <span class="label">USER EXPERIENCE</span> <span class="label">APPLICATION DESIGN</span> <span class="label">CSS</span> <span class="label">HTML</span> <span class="label">LESS</span> <span class="label">BOOTSTRAP</span><span class="label">USER TEST</span> <span class="label">USER RESEARCH</span> <span class="label">WIREFRAMING</span> <span class="label">PROTOTYPING</span>  <span class="label">PRODUCT MANAGER</span> <span class="label">AGILE</span>
                </div>


            </div>
        </div><!-- end row fluid -->



    </div><!-- end whoapium -->
    <p class="scroll text-center">
        <a class="hidden-phone" href="#contactus"><i class="next">&raquo;</i> Contact us</a>
        <a href="#"><i class="top">&raquo;</i> Go to the top</a></p>
</div><!-- end apiumwhofluid -->

<!-- CONTACT US APIUM -->
<div class="container-fluid apiumcontactfluid" id="contactus">
    <div class="container contactapium hero-unit">
        <h2 class="text-center">Questions? Contact us, it's free and worthy!</h2>
        <form class="form-horizontal">
            <div class="control-group">
                <label class="control-label" for="inputName">Name</label>
                <div class="controls">
                    <input type="text" id="inputName" placeholder="Name">
                </div>
            </div>
            <div class="control-group">
                <label class="control-label" for="inputEmail">Email</label>
                <div class="controls">
                    <input type="text" id="inputEmail" placeholder="Email">
                </div>
            </div>
            <div class="control-group">
                <label class="control-label" for="inputPhone">Phone</label>
                <div class="controls">
                    <input type="text" id="inputPhone" placeholder="Phone">
                </div>
            </div>
            <div class="control-group">
                <label class="control-label" for="inputMessage">Message</label>
                <div class="controls">
                    <textarea rows="8" type="text" id="inputMessage" placeholder="Message"></textarea>
                </div>
            </div>
            <div class="control-group">
                <div class="controls">
                    <button type="submit" class="btn btnapiumsend btn-primary">Send</button>
                </div>
            </div>
        </form>
    </div><!-- end contactapium -->
    <p class="scroll text-center">
        <a href="#"><i class="top">&raquo;</i> Go to the top</a></p>
</div><!-- end apiumcontactfluid -->




